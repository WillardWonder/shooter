...
document.addEventListener('keydown', e => {
  // Don't steal keys when typing in an input field
  if (document.activeElement && document.activeElement.tagName === 'INPUT') return;
  if (!joined) return;
  if (KEY_MAP[e.key]) {
    keys[KEY_MAP[e.key]] = true;
    e.preventDefault();
  }
});
...
document.addEventListener('mousedown', e => { if (e.button === 0 && joined) shooting = true; });
...
// Disable canvas interaction until in-game so lobby input works freely
